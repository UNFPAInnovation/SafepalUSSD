...
const menuOptions = {
	menuZero   : (req, res) => {
		...
		if (request_string === '6') {
			...
			res.status(403).json({ response_string: 'Something Went Wrong', action:'end' });
		}
	},
	menuOne   : (req, res) => {
        const { request_string } = req.body;
    ...
    menuThree: (req,res)=>{
        ...
        if(request_string==='1'||request_string==='2'||request_string ==='3'||request_string==='4'){
            ...
            res.status(200).json({ response_string: 'What\'s his/her Gender? \n 1. Male \n 2. Female', action:'request' });
        }
        else{
            res.status(403).json({ response_string: 'Invalid Input', action:'end' });
        }
    ...
    menuFour: (req,res)=>{
        ...
        if(request_string ==='1'||request_string ==='2'){
            for(let i=0;i<dataArray.length;i=i+1){
				if(dataArray[i].sessionID === req.body.session_id){
                    // capture Gender
                   if(request_string  === '1'){dataArray[i].gender = 'Male'}else{dataArray[i].gender = 'Female'};
					dataArray[i].menu = 5;
				}else{
					console.log('could not find User\'s SessionID');
				}
    ...
    menuFive: (req,res)=>{
        ...
        if(request_string==='1'||request_string==='2'){
            for(let i=0;i<dataArray.length;i=i+1){
				if(dataArray[i].sessionID === req.body.session_id){
                    // capture disability
                    if(request_string === '1'){dataArray[i].disability = 'No'}else{dataArray[i].disability = 'Yes'};
					dataArray[i].menu = 6;
				}else{
					console.log('could not find User\'s SessionID');
				}
			}
            res.status(200).json({ response_string: 'What happened to you? \n 1. Bad Touches \n 2. I was Raped \n 3. I was Defiled \n 4. Someone tried to raped me \n 5. Other', action:'request' });
        }
        else{
            res.status(403).json({ response_string: 'Invalid Input', action: 'end' });
        }
    ...
    menuSix:(req,res)=>{
        ...
        if(request_string==='1'||request_string==='2'||request_string==='3'||request_string==='4'||request_string==='5'){
            for(let i=0;i<dataArray.length;i=i+1){
				if(dataArray[i].sessionID === req.body.session_id){
            ...
            res.status(200).json({ response_string: 'Enter Your Age', action:'request' });
        }
        else{
            res.status(403).json({ response_string: 'Invalid Input', action: 'end' });
        }
    ...
    menuSeven:(req,res)=>{
        ...
        if(request_string.length>0){
            ...
            res.status(200).json({ response_string: 'Enter Location where it happened', action:'request' });
        }
        else{
            res.status(403).json({ response_string: 'Invalid Input' ,action:'end' });
        }
    ...
    menuEight:(req,res)=>{
        ...
        if(request_string.length>0){
            ...
            fs.writeFileSync('./src/reports.json',JSON.stringify(reports,null,2),'utf8');
            console.log(userReport);
            
            const safepalNum = Math.floor(Math.random()*90000); 
            res.status(200).json({ response_string: `Your SafePal Number is: ${safepalNum}...SafePal will contact you soon`, action:'end', userData:userReport, reports: reports });
